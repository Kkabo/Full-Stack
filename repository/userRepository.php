...
class UserRepository{
    ...
    function __construct(){
        $conn = new DatabaseConnection; //instanca
        $this->connection = $conn->startConnection(); //e thirrim funksionin startConnection qe ke te databaza
    }
    ...
    function insertUser($user){
        ...
        $statement = $conn->prepare($sql); //dergohet query ne kete variabel

        $statement->execute([$id,$fname,$lname,$username,$email,$password,$role]); //i pranon te dhenat

        echo "<script> alert('User has been inserted successfuly!'); </script>";

    }
    function getAllUsers(){   //i merr krejt te dhenat
        $conn = $this->connection;
